...
NBM model does not work for all data. Needs to be checked.
...
reg_mat = [[None for y in range(col)] for x in range(row)]
reg_mat_neighbors = [[None for y in range(col)] for x in range(row)]
reg_mat_clustering = [[None for y in range(col)] for x in range(row)]
...
for i in range(row):
	for j in range(col):
		...
		bm = sm.NegativeBinomial(indep_var_col,dep_var_mat,loglike_method='nb2')
		reg_mat[i][j] = bm.fit()
		"""
		...
		bm1 = sm.NegativeBinomial(indep_var_col,dep_var_mat_clustering,loglike_method='nb2')
		...
		pred_list = list(reg_mat[i][j].predict(features_mat))
		#print('list is:')
		#print(len(pred_list))
		#raw_input('stop')
		pred_neighbors_list = reg_mat_neighbors[i][j].predict(features_neighbors_mat)
		pred_neighbors_clustering_list = list(reg_mat_clustering[i][j].predict(features_neighbors_clustering_mat))
